...
unsigned int nStakeMinAge = 1 * 60 * 24 * 30; // minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 23; // stake age of full weight
unsigned int nStakeTargetSpacing = 1 * 60; // 60 seconds block spacing
int64 nChainStartTime = 1648177536;//2013/06/26/12:00:00
...
const string strMessageMagic = "CoinConnect Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // CoinConnect: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal coinconnectpro miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in CoinConnect
    bool fStrictPayToScriptHash = true; // Always active in CoinConnect
    ...
    // ppcoin: fees are not collected by miners as in coinconnectpro
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CoinConnect", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "March 24, 2021 - Invest In Coinconnect.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:CoinConnect genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xd67d0734790aac5a91941687643e0402a8293da05247edaa136d570b98a11a87"));
...
unsigned char pchMessageStart[4] = { 0xe2, 0xe4, 0xe6, 0xe8 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("CoinConnectMiner : proof-of-work not meeting target");

    //// debug print
    printf("CoinConnectMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CoinConnectMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("CoinConnectMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadCoinConnectMiner(void* parg);

static bool fGenerateCoinConnects = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void CoinConnectMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("coinconnectpro-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateCoinConnects || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateCoinConnects) && !fProofOfStake)
                return;
        }
        ...
        printf("Running CoinConnectMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadCoinConnectMiner(void* parg)
{
    ...
    try
    {
        ...
        CoinConnectMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadCoinConnectMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadCoinConnectMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadCoinConnectMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateCoinConnects(bool fGenerate, CWallet* pwallet)
{
    fGenerateCoinConnects = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateCoinConnects = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d CoinConnectMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadCoinConnectMiner, pwallet))
                printf("Error: NewThread(ThreadCoinConnectMiner) failed\n");
            Sleep(10);
        }
    }
}
